...
const ΠεριεχόμενοΓεγονότος = ({ στιγμή, γεγονόςΣεΠεριττόΑριθμό }) => {
  ...
  const φανερόΉΚρυφό = είδος.split(' ').includes('(Κρυφό)')
    ? 'κρυφό'
    : 'φανερό';
  ...
  const μεΉΧωρίςΔευτερεύουσαΓραμμή =
    όνομα || τύπος || φάσεις.length > 0
      ? 'με-δευτερεύουσα-γραμμή'
      : 'χωρίς-δευτερεύουσα-γραμμή';

  const μεΉΧωρίςΚείμενο = κείμενο ? 'με-κείμενο' : 'χωρίς-κείμενο';

  const μεΉΧωρίςΤεχνολογίες =
    τεχνολογίες.length !== 0 ? 'με-τεχνολογίες' : 'χωρίς-τεχνολογίες';
